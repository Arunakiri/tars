...
app.post('/api/start', (req, res) => {
	...
	if (!interval) {
		return res.status(400).json({ error: 'Interval is required (e.g., 5m, 30s)' });
	}
	...
	const scriptPath = path.join(__dirname, '..', 'chronos-frontend', 'chronos.sh');
...
app.post('/api/stop', (req, res) => {
	...
	try {
		...
		if (error.code !== 'ESRCH') {
			return res.status(500).json({ error: `Could not stop script: ${error.message}` });
		}
	}
